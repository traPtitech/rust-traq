...
pub async fn search_messages(
    ...
    offset: Option<i32>,
    sort: Option<&str>,
) -> Result<crate::models::MessageSearchResult, Error<SearchMessagesError>> {
    let local_var_configuration = configuration;
    ...
    if let Some(ref local_var_str) = word {
        local_var_req_builder =
            local_var_req_builder.query(&[("word", &local_var_str.to_string())]);
    }
    ...
    if let Some(ref local_var_str) = citation {
        local_var_req_builder =
            local_var_req_builder.query(&[("citation", &local_var_str.to_string())]);
    }
    ...
    if let Some(ref local_var_str) = has_url {
        local_var_req_builder =
            local_var_req_builder.query(&[("hasURL", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = has_attachments {
        local_var_req_builder =
            local_var_req_builder.query(&[("hasAttachments", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = has_image {
        local_var_req_builder =
            local_var_req_builder.query(&[("hasImage", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = has_video {
        local_var_req_builder =
            local_var_req_builder.query(&[("hasVideo", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = has_audio {
        local_var_req_builder =
            local_var_req_builder.query(&[("hasAudio", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = limit {
        local_var_req_builder =
            local_var_req_builder.query(&[("limit", &local_var_str.to_string())]);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<SearchMessagesError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
